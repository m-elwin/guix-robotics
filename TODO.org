* IMPLEMENT the ros_comm tests.
Some are additinoal unit tests
some might be itnegratino tests that we want to make a script
this will give more certianty that this works

rosbag downloads test data. Need to create a separate origin and use this as an input
and patch so the data is not downloaded.
